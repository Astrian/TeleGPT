...
const create_thread = async (topic: string, user: number, name: string)=> {
  ...
  await db_promise('INSERT INTO messages (id, user, thread, identity, content, sent_at) VALUES (?, ?, ?, ?, ?, ?)', [uuid.v4(), 0, thread_id, 'system', `A user created a new thread and you are about to chat with this user in this thread. The title of the thread is “${topic}”, this thread may be related to this topic. The name of the user is “${name}”.`, new Date().getTime()])
